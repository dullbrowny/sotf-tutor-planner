...
import Button from "../ui/Button.jsx";               // default export
import { Chip } from "../ui/Chip";                   // unchanged
import { Menu, MenuItem } from "../ui/Menu";         // unchanged
...
export default function ChatComposer({ onSend, defaultContext }) {
  ...
  // Accept seeded prompts from Admin → Overview (and anywhere else)
  useEffect(() => {
    const onSeed = (e) => {
      const { text: t, context: ctx, files: fs, mode: m } = e.detail || {};
      if (typeof t === "string") {
        setText((prev) => (prev && prev.trim() ? prev + "\n" + t : t));
      }
      ...
      setSeedToast("Prompt added");
      const id = setTimeout(() => setSeedToast(""), 1200);
      return () => clearTimeout(id);
    };
    window.addEventListener("sotf.chat.seed", onSeed);
    return () => window.removeEventListener("sotf.chat.seed", onSeed);
  }, []);
  ...
  return (
    <div className="card p-3 sticky bottom-0">
      ...
      <div className="flex items-end gap-2 relative">
        ...
        <button className="btn-ghost h-11 px-3" title="Voice (placeholder)">
          🎙️
        </button>
